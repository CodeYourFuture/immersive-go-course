...
func NewConsensusModule(id string, server *Server, storage Storage, ready <-chan interface{}, commitChan chan<- CommitEntry) *ConsensusModule {
	cm := new(ConsensusModule)
	cm.id = id
	cm.peerIds = make(map[string]bool)
	cm.server = server
	cm.storage = storage
...
func (cm *ConsensusModule) Submit(command CommandImpl) bool {
	cm.mu.Lock()
			...
			if err := cm.server.CallAppendEntries(peerId, args, &reply); err == nil {
				...
				if cm.state == Leader && savedCurrentTerm == reply.Term {
					if reply.Success {
						...
						cm.dlog("\nAppendEntries reply from %s !success: nextIndex := %d", peerId, cm.nextIndex[peerId])
					}
				}
			}
		}(peerId)
